...
export class FakeBackendInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        function authenticate() {
            ...
            if (!account) {
                // Generic message for non-existent accounts (security best practice)
                return error('Email or password is incorrect');
            }
            
            // For unverified accounts - only show verification message
            if (!account.isVerified) {
                const verifyUrl = `${location.origin}/account/verify-email?token=${account.verificationToken}`;
                return error(`
                    <div><strong>Email is not verified.</strong></div>
                    <br>
                    <div><strong>Verification Email.</strong></div>
                    Please click the link below to verify your email address:
                    <br><br>
                    <div><a href="${verifyUrl}">${verifyUrl}</a></div>
                `);
            }
            // For inactive accounts - show specific message
            if (account.status === 'Inactive') {
                return error('Your account is currently inactive. Please contact the administrator.');
            }            
            
            // Only check password for verified accounts
        ...
        // Update the error helper function to handle HTML
        function error(data) {
            if (typeof data === 'string') {
                return throwError({ error: { message: data } })
